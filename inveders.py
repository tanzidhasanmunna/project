...
def main():
    ...
    def redraw_window():
        ...
        lost_lable = main_font.render(" You Lost !! ", 1, (255, 255, 255))

        for enime in enemies:
            enime.draw(WIN)
    ...
    while run:
        ...
        if keys[pygame.K_LEFT] and player.x - player_val > 5:  # left
            player.x -= player_val
        if keys[pygame.K_RIGHT] and player.x + player_val + player.get_width() < WIDTH:  # right
            player.x += player_val
        if keys[pygame.K_UP] and player.y - player_val > 62:  # up
            player.y -= player_val
        if keys[pygame.K_DOWN] and player.y + player_val + player.get_height() < HEIGHT - 20:  # down
            player.y += player_val
        if keys[pygame.K_SPACE]:
            player.shoot()

        for enemy in enemies[:]:
            enemy.move(enemie_vel)
        ...
        player.move_laser(-laser_vel - 10, enemies)
